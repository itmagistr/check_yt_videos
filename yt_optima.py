            ...
            if opts.update == '3':
                ...
                if d is not None:
                    try:
                        ...
                        logging.info('exception on data logging json data: {}'.format(d))
                else:
...
def tagsUpdate(drv, vid, url, o):
    ...
    try:
        testElm = WebDriverWait(drv, float(o.timeout)).until(lambda x: x.find_element_by_class_name("stat-value-tripled-keywords"))
    except TimeoutException:
        logging.info("Превышено время ожидания загрузки страницы. Попытка обработать следующую ссылку.")
...
def getScores(dr, tdata):
    ...
    if len(elms) > 1:
        elm1 = elms[1].find_elements_by_xpath(".//span[@class='value-inner']")
        if len(elm1) > 0:
            tdata['tshow'] = float(elm1[0].text)
        else:
            tdata['tshow'] = -2
    else:
        tdata['tshow'] = -1
    ...
    try:
        ...
        logging.info("stat-value-tag-count Unexpected error: {}".format(sys.exc_info()[0]))
    ...
    try:
        ...
        logging.info("stat-value-tag-volume Unexpected error: {}".format(sys.exc_info()[0]))
    ...
    try:
        ...
        logging.info("stat-value-keywords-in-title Unexpected error: {}".format(sys.exc_info()[0]))
...
def rateWords(drv, opts):
    ...
    time.sleep(P100ms*30)  # пауза 3
    try:
        testElm = WebDriverWait(drv, float(opts.timeout)).until(lambda x: x.find_element_by_class_name("stat-value-tripled-keywords"))
    except TimeoutException:
...
def check_list(opts):
    ...
    odt = datetime.datetime.strptime(opts.dt, '%Y-%m-%d %H:%M')
    logging.info('---- дата свежести сохраненных данных {}'.format(odt))
    if urlslen > 0:
        driver = connect2Browser(opts)
    crow=1
    for u in urls:
        ...
        if svdata is not None and float(opts.seo) <= float(svdata["seo"]):
            ...
            try:
                myElem = WebDriverWait(driver, float(opts.timeout)).until(lambda x: x.find_element_by_class_name("stat-value-tripled-keywords"))
            except TimeoutException:
            ...
            elm=driver.find_element_by_class_name('stat-value-seo-score')
            elm1 = elm.find_elements_by_xpath(".//span[@class='value-inner']")[0]
            #elm = driver.find_elements_by_xpath("//span[contains(@class,'stat-value-seo-score')]")[0] #13.5/50
            #elm1 = elm.text.split('/')[0] #13.5
            seotext = elm1.text.split('/')[0] #VIDIQ SEO SCORE
            logging.info('seo {}'.format(seotext))
            if isxls:
                wsd.cell(row=crow, column=4, value=float(seotext))
            cdata['seo'] = seotext
            ...
            seotext1 = elm1.text.split('/')[0] # Actionable
            if isxls:
                wsd.cell(row=crow, column=5, value=float(seotext1))
            cdata['stat-value1'] = seotext1

            if len(elms) > 1:
                elm1 = elms[1].find_elements_by_xpath(".//span[@class='value-inner']")[0]
                #logging.info('undefined {}'.format(elm1.text))
                seotext2 = elm1.text.split('/')[0] # Perfomance
                if isxls:
                    wsd.cell(row=crow, column=11, value=float(seotext2))
                cdata['stat-value2'] = seotext2
            else:
                cdata['stat-value2'] = -1 # не найдено
...
def importTags(opts):
    ...
    for vid, curvid in urls.items():
        ...
        for tagelm in tags:
            ...
            cdata[t]['seo'] = float(elm1.text)
            cnt_retry = 1
            while ((cdata[t]['seo'] < 0.01) or (cdata[t]['seo'] > MaxScore)) and cnt_retry < 5:
                cnt_retry +=1
                # если не успело значение обновиться, то подождем еще и вычитаем повторно
                time.sleep(P100ms*20) # пауза 2
                #get SEO score
...
def getRankedTags_V2(vid):
    ...
    with orm.db_session:
        ...
        for tg in TagSEO.select(lambda t: t.vid==vid and t.real > 6.7 and t.real < 7.1).order_by(orm.raw_sql('real desc, seo desc, length(tag)')):
            #if tg.tag not in tgs:
            tgs+=[tg.tag]
    return tgs
    ...
    for u in urls:
        ...
        with orm.db_session():
            ...
            if likeslen == 0:
                ...
                # без лайка $x('//lottie-component/*[name()="svg"]/*[name()="g"]/*[name()="g" and @style="display: none;" and contains(@clip-path, "url(#__lottie_element_18)")]')
                # лайк установлен $x('//lottie-component/*[name()="svg"]/*[name()="g"]/*[name()="g" and @style="display: block;" and contains(@clip-path, "url(#__lottie_element_18)")]')
                ...
                if len(like_btns) > 0:
                    ...
                    # btns = driver.find_elements_by_xpath('//div[@id="segmented-like-button"]/ytd-toggle-button-renderer/yt-button-shape/button')
                    # if len(btns) > 0:
                    #    btns[0].click()
                    ...
                    # не захотело ставить лайк
                    #driver.find_elements_by_xpath('//yt-animated-icon[@animated-icon-type="LIKE"]')[0].click()
